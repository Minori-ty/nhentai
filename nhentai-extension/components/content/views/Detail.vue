...
import { getGalleryInfo, favoriteGallery } from '@/api/index'
import type { IGallery, Tag } from '@/api/info.d'
import type { ContentMessage } from '@/types/messages.d'
import { handleImageError } from '@/utils/imageFallback'
import { TagTypeEnum, LangEnum, MsgTypeEnum } from '@/enums'
...
function handleProgressMsg(msg: ContentMessage) {
    if (msg.id !== gallery.value?.id) return
    if (msg.type === MsgTypeEnum.Progress) {
        downloadProgress.value = msg.data
    }
...
onMounted(async () => {
    ...
    try {
        ...
        downloaded.value = await MediaService.hasMedia(id)
    } finally {
        loading.value = false
    }
})
...
<template>
    ...
    <template v-else-if="gallery">
        <!-- ===== 上半部分：左封面 + 右信息 ===== -->
        <div class="mx-auto mb-6 flex max-w-[1110px] gap-8 rounded-xl bg-[#2A3744] px-6 py-8">
            <!-- 左边：封面 -->
            <div class="shrink-0">
                <img
                    :src="coverUrl"
                    :alt="gallery.title.japanese || gallery.title.english"
                    class="w-86 rounded-lg shadow-lg"
            ...
            <div class="flex min-w-0 flex-col gap-4">
                ...
                <div class="mt-2 flex items-center gap-3 self-start">
                    ...
                    <!-- 下载按钮 -->
                    <BaseBtn
                        v-if="!downloaded && !downloadProgress"
                        variant="info"
                        size="semibold"
                    ...
                    <!-- 下载中：百分比 -->
                    <BaseBtn
                        v-else-if="downloadProgress > 0 && downloadProgress < 100"
                        variant="warning"
                        size="semibold"
                        disabled
                    >
                    ...
                    <!-- 已下载 -->
                    <span v-else class="rounded-md bg-green-400 px-5 py-2 text-base font-semibold text-white">
                        ✓ 已下载
                    </span>
        ...
        <div class="mx-auto w-fit rounded-xl bg-[#2A3744] px-6 py-6">
            <div class="grid grid-cols-5 justify-center gap-6">
                <div v-for="page in gallery.pages" :key="page.number" class="group">
                    <div class="mx-auto h-80 w-56.25 overflow-hidden rounded-lg bg-gray-800">
                        <img
                    ...
                    <p class="mx-auto mt-2 w-56.25 text-center text-sm text-gray-400">#{{ page.number }}</p>
                </div>
            </div>
        </div>
    </template>
